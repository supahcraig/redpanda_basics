`redpanda.yaml` is a frustrating thing at times.  There is no one complete reference for everything you can do.   A prime example is the `insecure_skip_verify = true` setting which is needed for self-signed certs when using TLS.  This is nowhere in the public facing docs.


This link is the closest thing to a full set of config options.  At the very least it shows the sections & syntax.
