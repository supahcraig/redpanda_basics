...
**PHASE 2**: We build out some json that will be passed to the Redpanda control plane API to create the Redpanda Network and Redpand Cluster (which are really just logical "things" within Redpanda Cloud, not the actual network/resources in AWS).  The cluster create step is where you specify single- or multi-az, by setting the REDPANDA_ZONES env var.  The output from these API calls is essentially to just return the ID's of the network & cluster.  You can see the network in the cloud UI, but the cluster won't be there until into phase 3.   
